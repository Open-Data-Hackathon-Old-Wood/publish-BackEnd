## app/features/pin_detail/nara/schemas.py
...
class NaraDetailQuery(BaseModel):
...
# ナラ枯れ詳細のテーブルから詳細情報を返すためのスキーマ.詳細情報は１つのPinに対してしか返さないことを前提としている
class NaraDetailOut(BaseModel):
    point_id: int
    needle_size: int = Field(..., description="Needle size of the pine")
...
class NaraDetailResponse(BaseModel):
    id: str
    ...
    texture_rating: Optional[int] = Field(default=None,serialization_alias="textureRating")
    hole_size: int = Field(serialization_alias="holeSize")
