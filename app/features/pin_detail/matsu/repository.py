## app/features/pin_detail/matsu/repository.py
...
class MatsuDetailRepository:
    def __init__(self, db: Session):
        self.db = db
    
    # 期間を指定して、指定された位置情報のピンIDを取得するメソッド。最新の一件を取得.ただしカテゴリーも検索対象に指定する
    def fetch_point_id(self, *, category:str = "Matsu" ,lng: float, lat: float, start:str, end:str):
    ...
    def fetch_point_detail(self, *, point_id: int):
        where_id = (Matsu.point_id == point_id)
        stmt = (
            select(
                Matsu.point_id,
                Matsu.health_status,  
    ...
    # def fetch_point_detail(self, *, point_id: int):
    #     where_id = (Matsu.point_id == point_id)
    #     stmt = (select(Matsu).where(where_id).limit(1))
    #     obj = self.db.execute(stmt)
    #     print(obj.